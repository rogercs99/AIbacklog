...
ports=(13307 12309 12321 12322 12323 18080 18081 18082 18100)
listeners=$(ss -H -lnt)
for p in "${ports[@]}"; do
...
# Docker-published Habbo ports must remain loopback-only.
...
# Habbo must not be exposed directly through nginx; Cloudflare tunnel is the public ingress.
...
grep -q 'service: http://127.0.0.1:18100' "$CF" || fail 'Cloudflare v0.8.5 web route missing'
grep -Fq 'path: ^/v31-websockify(/.*)?$' "$CF" || fail 'Cloudflare V31 websocket path missing'
grep -q 'service: http://127.0.0.1:18131' "$CF" || fail 'Cloudflare V31 websocket service missing'
grep -Fq 'path: ^/r39-websockify(/.*)?$' "$CF" || fail 'Cloudflare R39 websocket path missing'
grep -q 'service: http://127.0.0.1:18139' "$CF" || fail 'Cloudflare R39 websocket service missing'
