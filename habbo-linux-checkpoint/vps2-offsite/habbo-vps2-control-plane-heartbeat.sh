...
mapfile -t failed < <(systemctl --failed --no-legend --plain 2>/dev/null | awk '{print $1}' | grep -E '^habbo-(vps1-offsite|public-webkit|vps2-control-plane)' || true)
((${#failed[@]}==0)) || issues+=("failed_units=${failed[*]}")
if ((${#issues[@]}==0)); then
  result=success
  detail="timers=${healthy}/4-enabled+active failed_units=0 root_free_kb=$root_free_kb shm_free_kb=$shm_free_kb offsite_store=${offsite_archives}/3-deep"
else
  result=failed
...
printf 'validated_at_utc=%s\nresult=%s\ntimers_total=4\ntimers_healthy=%s\nfailed_units=%s\nroot_free_kb=%s\nshm_free_kb=%s\noffsite_store_healthy=%s\noffsite_deep_verified=%s\noffsite_archives=%s\ndetail=%s\n' \
  "$now" "$result" "$healthy" "${#failed[@]}" "$root_free_kb" "$shm_free_kb" "$offsite_store_healthy" "$offsite_deep_verified" "$offsite_archives" "$detail" >"$marker"
