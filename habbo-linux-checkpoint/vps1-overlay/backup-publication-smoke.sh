...
LATEST_FILE=$ROOT/LATEST_PUBLIC_WEB_BACKUP
...
[[ "$latest" =~ ^/srv/habbo/backups/manual-[0-9]{8}T[0-9]{6}Z$ ]] || fail "LATEST path malformed: $latest"
...
[[ -e /run/lock/habbo-backup.lock ]] || fail 'backup lock file missing'
command -v flock >/dev/null || fail 'flock unavailable'

echo 'PASS: Habbo backup publication smoke'
echo "latest=$latest manifest=relative+verified staging_stale=0 latest_tmp_stale=0 lock=present"