...
fetch() {
  curl --retry 4 --retry-all-errors --retry-delay 1     --connect-timeout 5 --max-time 15 -fsS     -H 'Cache-Control: no-cache' -H 'Pragma: no-cache' -A "$UA" "$@"
}
TMP=$(mktemp)
trap 'rm -f "$TMP"' EXIT
fetch "$BASE/" >"$TMP"
...
python3 /srv/habbo/ops/public-web-direct-assets-audit.py
