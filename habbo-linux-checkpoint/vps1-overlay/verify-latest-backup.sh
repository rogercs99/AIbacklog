...
[[ "$(wc -l < "$B/vps2-control-plane-files-sha256.txt")" -eq 35 ]] || { echo 'FAIL: VPS2 control-plane recovery manifest line count mismatch' >&2; exit 1; }
...
test -x "$WORK/ops/live-drift-remote-smoke.sh"
...
test -x "$WORK/ops/postboot-failed.sh"
test -f "$B/habbo-postboot-validate.service"
test -f "$B/habbo-postboot-validate-failed.service"
grep -Fq 'OnFailure=habbo-postboot-validate-failed.service' "$B/habbo-postboot-validate.service"
