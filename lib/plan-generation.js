...
function deduplicateItems(items) {
  ...
  items.forEach((item) => {
    ...
    let titleKey = stripTypePrefix(item.title);
    if (!titleKey) {
      titleKey = item.title || `${item.type || "Item"}`;
    }
    ...
    const key = `${item.type}|${parentKey}|${titleKey.toLowerCase()}`;
    if (seen.has(key)) {
      return;
    }
    seen.add(key);
        ...
        if (chunkResult?.items?.length) {
          ...
          items.forEach((item) => {
            ...
            if (item.parent_external_id) {
              item.parent_external_id = `T-${String(
                Math.max(1, counter - 1),
              ).padStart(3, "0")}`;
            }
            if (item.type !== "Epic" && !item.parent_external_id && merged.items.length > 0) {
              const epicCandidate = merged.items.find((it) => it.type === "Epic");
              if (epicCandidate) {
                item.parent_external_id = epicCandidate.external_id;
              }
            }
            if (item.type !== "Epic" && !item.parent_external_id && merged.items.length > 0) {
              const nonEpic = merged.items[merged.items.length - 1];
              item.parent_external_id = nonEpic.external_id;
            }
            const area = String(item.area || "other");
            item.area = area.split(/[|,]/)[0]?.trim() || "other";
            item.priority = item.priority || "Medium";
