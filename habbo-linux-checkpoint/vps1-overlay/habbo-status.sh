...
if [[ -f /srv/habbo/VPS2_CONTROL_PLANE_STATUS ]]; then
  ...
  vps2cp_age=999999999; vps2cp_result=missing; vps2cp_total=0; vps2cp_healthy=0; vps2cp_failed=999; vps2cp_root=0; vps2cp_shm=0; vps2cp_store=0; vps2cp_archives=0; vps2cp_ok=false
fi
...
[[ "$vps2cp_store" == 1 && "$vps2cp_archives" == 3 ]] || vps2cp_ok=false
...
if [[ "$vps2cp_store" == 1 && "$vps2cp_archives" == 3 ]]; then vps2cp_store_label=OK; else vps2cp_store_label=FAIL; fi
